...
export async function runReconciliation(batchId, uploadBatchId = null) {
  ...
  for (const order of orders) {
    ...
    // LAYER 4: Tax-line verification — Section 194-O TDS + GST-on-fee
    let taxIssues = [];
    if (status === 'matched' && transfer) {
      taxIssues = await verifyTaxLines(order, transfer);
      if (taxIssues.length > 0) {
        status = 'exception';
        category = 'tax_line_discrepancy';
        context = taxIssues.map((i) => `${i.line}: expected ₹${(i.expected / 100).toFixed(2)}, actual ₹${(i.actual / 100).toFixed(2)} (${i.rule})`).join(' | ');
      }
    }

    reconRecords.push({
      order, transfer, status, category, context, expectedSettled, actualSettled, taxIssues,
    });
  }
