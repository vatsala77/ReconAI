...
export async function runReconciliation(batchId, uploadBatchId = null) {
  ...
  for (const order of orders) {
    ...
    const transferSourceType = transfer?.notes && typeof transfer.notes === 'object' ? transfer.notes.sourceType : null;
    const isLiveRazorpayTransfer = transferSourceType === 'razorpay';
    ...
    } else if (isLiveRazorpayTransfer && transfer.onHold && transfer.onHoldUntil && transfer.onHoldUntil < Math.floor(Date.now() / 1000)) {
    ...
    } else if (isLiveRazorpayTransfer && transfer.onHold && !transfer.onHoldUntil) {
      status = 'exception'; category = 'chargeback_hold';
      context = 'Transfer is currently on hold and not released yet';
      actualSettled = transfer.amount;
