...
async function explainDiscrepancy(data) {
  // Step 1: Clean query generation to optimize RAG vector weights
...
Respond ONLY with a JSON object matching the required schema. No markdown, no preamble.`;
  ...
  try {
    ...
    console.error('❌ AI reconciliation explainer processing engine failed:', error);

    // Safely fallback to native mathematical variance execution in paise if Gemini drops out
