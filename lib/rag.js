  ...
  async init() {
    ...
    for (const file of files) {
      ...
      for (const segmentText of segments) {
        let embedding;
        if (embeddingCache.has(segmentText)) {
          embedding = embeddingCache.get(segmentText);
        } else {
          embedding = await this.getEmbedding(segmentText);
          this.cacheEmbedding(segmentText, embedding);
        }
...
async function warmUpRAG() {
  const rag = new SimpleRAG();
  await rag.init();
  return rag;
}

module.exports.warmUpRAG = warmUpRAG;