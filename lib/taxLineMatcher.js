...
// Cache for cumulative annual amounts to avoid repeated DB queries
const cumulativeAmountCache = new Map();
...
async function getCumulativeAnnualAmount(sellerId, orderDate) {
  ...
  const cacheKey = `${sellerId}_${start.toISOString()}_${end.toISOString()}`;
  
  if (cumulativeAmountCache.has(cacheKey)) {
    return cumulativeAmountCache.get(cacheKey);
  }
  ...
  cumulativeAmountCache.set(cacheKey, amount);
  return amount;
}
